...
local Window = Fluent:CreateWindow({
    Title = "ĐẠT 09 CẬN HUYẾT",
    SubTitle = "Version 1.0",
    TabWidth = 160,
    Size = UDim2.fromOffset(500, 320),
    Acrylic = true,
    Theme = "Darker",
    MinimizeKey = Enum.KeyCode.End xcd
})
local Tabs = {
...
ImageButton.Image = "https://www.roblox.com/catalog/17330249020"
...
local listfastattack = {'Normal Attack','Fast Attack','Super Fast Attack'}
    ...
    _G.FastAttackkhangmod_Mode = Value
	if _G.FastAttackkhangmod_Mode == "Fast Attack" then
		_G.Fast_Delay = 0.1
	elseif _G.FastAttackkhangmod_Mode == "Normal Attack" then
		_G.Fast_Delay = 0.15
	elseif _G.FastAttackkhangmod_Mode == "Super Fast Attack" then
		_G.Fast_Delay = 0
	end
end)
...
while wait() do
    pcall(function()
        if _G.FastAttackkhangmod then
            repeat wait(_G.DelayAttackPlayer)
               AttackNoCoolDown()
            until not _G.FastAttackkhangmod
        end
    end)
end
end)
...
----------------------------------------------------------------------------------------------------------------------------------------------
Fluent:Notify({
    Title = "Đạt 09 lor Version",
    Content = "Cảm Ơn Bạn Đã Dùng Script ",
    Durtion = 3
})
warn("Khang Mod Game Vetsion 1.0 Loading Completed !!")